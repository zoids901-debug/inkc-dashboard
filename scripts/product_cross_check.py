...
def main():
    ...
    for year in ['2021', '2022', '2023', '2024', '2025', '2026']:
        ...
        for store in stores:
            # 운정은 토스(TOSS) 매장 — OK포스 raw 없음. 아래에서 토스 원본과 별도 대조.
            if store == '운정':
                continue
    ...
    # 4) 운정 — 토스(TOSS) 매장. OK포스 raw 없음 → 토스 API 원본과 직접 대조.
    unjeong_prod = {y: n for (y, s), n in prod_sy.items() if s == '운정' and n > 0}
    if unjeong_prod:
        try:
            ...
            while d <= end:
                try:
                    n = toss_day_net(hdr, d.isoformat())['net']
                except Exception:
                    n = 0
                ys = str(d.year)
                toss_year[ys] = toss_year.get(ys, 0) + n
                d += timedelta(days=1)
            for y in sorted(unjeong_prod):
                pn = unjeong_prod[y]
                rn = toss_year.get(y, 0)
                diff = pn - rn
                base = max(rn, pn) or 1
                pct = abs(diff) / base * 100
                if rn == 0:
                    level, msg = "warn", "토스 원본 조회 0원 — 확인 필요"
                elif abs(diff) < ABS_MIN:
                    level, msg = "ok", "토스 원본과 일치"
                elif pct >= PCT_BAD:
                    level, msg = "bad", "큰 차이 — 운정 backfill 누락/오류 의심"
                elif pct >= PCT_WARN:
                    level, msg = "warn", "1% 이상 차이 — 부분 누락 가능"
                else:
                    level, msg = "ok", "미세한 차이 (진행 중인 해 시차 등)"
            ...
            print(f"  운정 토스 대조: {sorted(toss_year.items())}")
        except Exception as e:
            print(f"  ! 운정 토스 검증 실패: {e}")
